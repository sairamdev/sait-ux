function WMSTiled(wmsTiledOptions) {
	var options = {
		getTileUrl : function(coord, zoom) {
			...
			var top = proj.fromPointToLatLng(new google.maps.Point(coord.x
					* 256 / zfactor, coord.y * 256 / zfactor));
			var bot = proj.fromPointToLatLng(new google.maps.Point(
					(coord.x + 1) * 256 / zfactor, (coord.y + 1) * 256
							/ zfactor));
...
function WMSFeatureInfo(mapObj, options) {
	...
	this.fixedParams = 'REQUEST=GetFeatureInfo&EXCEPTIONS=application%2Fjson&SERVICE=WMS&FEATURE_COUNT=50&styles=&srs=EPSG:3857&INFO_FORMAT=application%2Fjson&format=image%2Fpng';
...
WMSFeatureInfo.prototype.getUrl = function(coord) {
	...
	rUrl += '&BBOX=' + bbox;
	rUrl += '&WIDTH=' + this.map.getDiv().clientWidth + '&HEIGHT='
			+ this.map.getDiv().clientWidth;
